            ...
            if guiObject:GetAttribute("enabled") == true then
                if time == "opening" then
                    if guiObject:IsA("Frame") then
                        TweenService:Create(guiObject,TweenInfo.new(module.TweenLengthOther, Enum.EasingStyle.Sine,Enum.EasingDirection.Out), {Size = guiObject:GetAttribute("sizeStart"), BackgroundTransparency = num}):Play()
                    end
                    if guiObject:IsA("ImageLabel") then
                        TweenService:Create(guiObject,TweenInfo.new(module.TweenLengthOther, Enum.EasingStyle.Sine,Enum.EasingDirection.Out), {Size = guiObject:GetAttribute("sizeStart"), ImageTransparency = num}):Play()
                    end
                end
